document.addEventListener("DOMContentLoaded", () => {
  ...
  // Add event listener for the Find More Games button
  const findMoreGamesBtn = document.getElementById("find-more-games-btn")
  if (findMoreGamesBtn) {
    findMoreGamesBtn.addEventListener("click", () => {
      window.location.href = "../home.html"
    })
  }

  // Add event listener for the Share Results button
  ...
  if (shareButton) {
    shareButton.addEventListener("click", () => {
      ...
      // Create share text
      const shareText = `I scored ${userScore} points on today's Impossi.World quiz! Can you beat my score?`

      // Check if the Web Share API is available
      if (navigator.share) {
        navigator
          .share({
            title: "My Impossi.World Score",
            text: shareText,
  ...
  function showAlreadyPlayedPopup() {
    ...
    if (scoresData) {
      ...
      if (scores[quizDate] && scores[quizDate].length > 0) {
        ...
        const userScore = scores[quizDate][scores[quizDate].length - 1]
        document.getElementById("already-played-score").textContent = userScore
      }
    }
